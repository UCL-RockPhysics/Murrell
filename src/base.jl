...
function M_reduce!(P,exp_info; stresscorr=true)
    ...
    P[:F_kN_c][I3+1:I4] = F_smooth[I3+1:I4] .-(P[:Pc_corr][I3].-P[:Pc_corr][I3+1:I4]) .+P[:F_kN_c][I3] # Correct unload for offset resulting from friction until out of contact
    P[:F_kN_c][I4+1:end] = F_smooth[I4+1:end] .-(P[:Pc_corr][I4].-P[:Pc_corr][I4+1:end] ).+P[:F_kN_c][I4] # Now differential load is given by seal friction, correct for this
